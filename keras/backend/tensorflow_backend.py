...
def rnn_mut(step_function, inputs, plain_inputs, initial_states,
        go_backwards=False, mask=None, constants=None,
        unroll=False, input_length=None):
    """Iterates over the time dimension of a tensor.
    ...
    if unroll:
        ...
        if mask is not None:
            ...
            for inp, mask_t in zip(input_list, mask_list):
                output, new_states = step_function(inp, plain_inputs, states + constants)
                if getattr(output, '_uses_learning_phase', False):
                    uses_learning_phase = True
            ...
            for inp in input_list:
                output, states = step_function(inp, plain_inputs, states + constants)
                if getattr(output, '_uses_learning_phase', False):
                    uses_learning_phase = True
                successive_outputs.append(output)
                successive_states.append(states)
        ...
        time_steps = tf.shape(inputs)[0]
        outputs, _ = step_function(inputs[0], plain_inputs, initial_states + constants)
        output_ta = tensor_array_ops.TensorArray(
            dtype=outputs.dtype,
            size=time_steps,
            tensor_array_name='output_ta')
